...
# 소개
인스타그램의 디자인과 기능을 참고하여 개발한 사이트 입니다. <br>
피드 작성, 좋아요, 댓글 작성, 검색, 팔로우 ,실시간 알림, 메세지 기능을 구현한 사이트 입니다.
...
<UL>
 ...
 <LI>자신의 피드를 제외한 최신 피드를 감상 할 수 있습니다.</LI>
...
![프로필 이미지 폴더](https://github.com/oals/PortfolioSocialNetWork-backEnd/assets/136543676/012d0515-5c24-46e6-bc37-4b430725e6f1)
...
<li>여러 이미지를 업로드 할 수 있도록 구현 했고 게시물의 내용과 태그를 구분해서 입력 받도록 구현 했습니다.</li>
...
<li>피드의 좋아요를 엔티티로 관리함으로써 중복 좋아요 클릭을 방지하였습니다.</li>
<li>좋아요 버튼 클릭 시 해당 피드의 좋아요 수가 실시간으로 변경되도록 구현했습니다.</li>
...
<li>태그 클릭 및 태그 검색 시 태그 엔티티를 통해 연관 관계에 있는 피드들을 가져오도록 구현했습니다. </li>
<li>검색 결과가 없을 시 안내 메세지를 표시하도록 구현 했습니다.</li>
</UL>
...
<li>팔로우 관계에 있는 유저만 메세지를 보낼 수 있도록 구현했습니다.</li>
<li>팔로우,팔로워,맞팔로우 관계에 따라 텍스트가 다르게 표시 되도록 구현 했습니다. </li>
</UL>
...
<li> 이미 읽은 알림은 text-secondary 속성을 통해 흐리게 만들고 새로운 알림은 fw-bold를 통해 진하게 표시 되도록 구현했습니다. </li>
<li> 해당 알림 클릭 시 알림의 종류에 따른 위치로 이동 되도록 구현 했습니다.</li>
</UL>
...
<li>제일 마지막으로 메세지를 주고 받은 채팅방이 상단에 나타나도록 구현했습니다. </li>
...
리엑트를 사용해 프론트엔드를 구현했다.<br>
아무래도 개발을 하면서 공부하니 더 이해가 빠르게 된 것 같다<br>
처음에 부모 자식 간의 관계를 애매하게 설정해서 후에 컴포넌트 간의 관계를 다시 설정했다 하하.. <br>
하지만 이로 인해 다음 프로젝트에서는 어떻게 부모 자식 관계를 설정해서 코드 재사용성을 늘릴지에 대해 좀 알게 된 것 같다 <br>
